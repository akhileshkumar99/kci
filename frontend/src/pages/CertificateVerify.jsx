...
import { Search, Award, CheckCircle, XCircle, ShieldCheck, UserCheck } from 'lucide-react';
...
export default function CertificateVerify() {
  ...
  const runVerification = async (keyToVerify) => {
    const query = (keyToVerify || certNumber).trim();
    if (!query) return toast.error('Please enter a Certificate No., Roll No., or Form No.');

    setLoading(true);
    setSearched(true);
    try {
      // Pass search key both in URL param and query params for max compatibility
      const { data } = await api.get(`/certificates/verify/${encodeURIComponent(query)}?roll=${encodeURIComponent(query)}&formNo=${encodeURIComponent(query)}`);
  ...
  useEffect(() => {
    if (queryKey) {
      setCertNumber(queryKey);
      runVerification(queryKey);
    }
  }, [queryKey]);

  const handleVerify = (e) => {
    e.preventDefault();
    runVerification(certNumber);
  };
      ...
      <section className="relative bg-slate-950 text-white py-14 sm:py-16 text-center overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative max-w-3xl mx-auto px-4">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/15 rounded-full px-4 py-1.5 text-xs sm:text-sm mb-4 text-amber-300 shadow-md">
            <ShieldCheck className="w-4 h-4 text-amber-400" /> Digital Identity & Certificate Verification
          </div>
          <h1 className="text-3xl sm:text-5xl font-black mb-3 tracking-tight">
            Verify <span className="bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-200 bg-clip-text text-transparent">Student Record</span>
          </h1>
          <p className="text-gray-300 text-sm sm:text-base max-w-lg mx-auto font-normal leading-relaxed">
            Enter Certificate No., Roll No., or Form No. to verify student credentials & certificate authenticity
          </p>
        </motion.div>
      </section>
            ...
            <form onSubmit={handleVerify} className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Enter Certificate No., Roll No., or Form No. (e.g. KCI20260001)"
              ...
              <button type="submit" disabled={loading} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-extrabold rounded-xl text-xs sm:text-sm shadow-md hover:shadow-lg transition-all flex items-center gap-2 shrink-0 cursor-pointer">
              ...
              {certificate ? (
                <div className="bg-white rounded-3xl shadow-lg p-6 sm:p-8 border-2 border-emerald-200">
                  <div className="flex items-center gap-3 mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-200/80">
                    ...
                    <div>
                      <div className="font-extrabold text-emerald-900 text-base">
                        {certificate.isStudentRecord ? 'Student Identity Verified ✓' : 'Certificate Verified ✓'}
                      </div>
                      <div className="text-emerald-700 text-xs font-medium">
                        Official KCI Recognized Student Record & Credentials
                      </div>
                    </div>
                  </div>

                  <div className="text-center mb-6 py-4 bg-slate-50 rounded-2xl border border-gray-100">
                    {certificate.isStudentRecord ? (
                      <UserCheck className="w-14 h-14 text-blue-600 mx-auto mb-2" />
                    ) : (
                    ...
                    <p className="text-gray-500 text-xs mt-1">
                      {certificate.isStudentRecord ? 'Verified Student of Keerti Computer Institute' : 'has successfully completed the program'}
                    </p>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs sm:text-sm">
                    {[
                      { label: 'Certificate / Ref No.', value: certificate.certificateNumber },
                      { label: "Father's Name", value: certificate.fatherName || '—' },
                      { label: 'Roll Number', value: certificate.rollNumber || '—' },
                      { label: 'Form Number', value: certificate.formNo || '—' },
                      { label: 'Issue / Admission Date', value: certificate.issueDate ? new Date(certificate.issueDate).toLocaleDateString('en-IN') : '—' },
                      { label: 'Record Status', value: certificate.isValid ? '✓ Official Valid Record' : '✗ Unverified' },
                    ].map(({ label, value }) => (
                ...
                <div className="bg-white rounded-3xl shadow-md p-8 text-center border-2 border-red-100">
                  ...
                  <h3 className="text-lg font-extrabold text-gray-900 mb-1">Record Not Found</h3>
                  <p className="text-gray-500 text-xs sm:text-sm max-w-md mx-auto font-normal">
                    No matching record was found for "{certNumber}". Please verify the number or contact KCI support.
                  </p>
                </div>
              )}
            </motion.div>
