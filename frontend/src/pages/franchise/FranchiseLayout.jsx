...
const navItems = [
  { path: '/franchise', label: 'Dashboard', icon: LayoutDashboard, exact: true, color: 'text-blue-400' },
  ...
  { path: '/franchise/study-material', label: 'Study Material', icon: Library, color: 'text-violet-400' },
  { path: '/franchise/notifications', label: 'Notifications', icon: Bell, color: 'text-blue-400' },
  { path: '/franchise/contacts', label: 'Messages', icon: MessageSquare, color: 'text-rose-400' },
];
...
export default function FranchiseLayout() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'info', title: 'New Admission', msg: 'A new student admission received', time: '2m ago', read: false },
    { id: 2, type: 'success', title: 'Student Added', msg: 'New student successfully registered', time: '1h ago', read: false },
    { id: 3, type: 'warning', title: 'Result Pending', msg: 'Results for batch 2026 pending', time: '3h ago', read: true },
  ]);
  ...
  const navHover = dark ? 'hover:bg-gray-800 hover:text-white' : 'hover:bg-emerald-50 hover:text-emerald-700';
      ...
      >
        ...
        <div className={`h-16 flex items-center px-4 border-b ${dark ? 'border-gray-800' : 'border-gray-100'} shrink-0`}>
          <motion.div whileHover={{ scale: 1.05 }} className="relative shrink-0">
            <img src="/logo.png" alt="KCI" className="w-9 h-9 rounded-xl object-cover shadow-md ring-2 ring-emerald-500/30" />
          ...
          <AnimatePresence>
            {sidebarOpen && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="ml-3 overflow-hidden">
                ...
                <div className="text-emerald-500 text-[10px] font-bold tracking-wider whitespace-nowrap">Franchise Panel</div>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className={`p-3 border-t ${dark ? 'border-gray-800' : 'border-gray-100'} space-y-1`}>
          <button onClick={() => setDark(!dark)}
            ...
            {dark ? <Sun className="w-5 h-5 shrink-0 text-yellow-400" /> : <Moon className="w-5 h-5 shrink-0 text-emerald-400" />}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className={`h-16 ${header} border-b flex items-center px-4 gap-3 shrink-0 shadow-sm`}>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {/* Dark/Light toggle */}
            <button onClick={() => setDark(!dark)}
              className={`p-2 rounded-xl transition-colors ${dark ? 'text-yellow-400 hover:bg-gray-800' : 'text-emerald-500 hover:bg-gray-100'}`}>
            ...
            <div className="relative" ref={notifRef}>
              <button onClick={() => { setNotifOpen(!notifOpen); setProfileOpen(false); }}
                ...
                {unread > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 bg-emerald-500 rounded-full text-white text-[9px] flex items-center justify-center font-bold animate-pulse">
                    {unread}
                  </span>
                )}
              </button>
                ...
                {notifOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    ...
                    <div className={`flex items-center justify-between px-4 py-3 border-b ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                      ...
                      <button onClick={markAllRead} className="text-xs text-emerald-500 hover:text-emerald-600 font-medium">Mark all read</button>
                    </div>
                    <div className="max-h-72 overflow-y-auto">
                      {notifications.map(n => {
                        const Icon = { info: Info, success: CheckCircle, warning: AlertCircle }[n.type] || Info;
                        return (
                          <div key={n.id} className={`flex items-start gap-3 px-4 py-3 border-b last:border-0 transition-colors ${dark ? 'border-gray-800 hover:bg-gray-800' : 'border-gray-50 hover:bg-gray-50'} ${!n.read ? (dark ? 'bg-emerald-950/30' : 'bg-emerald-50/50') : ''}`}>
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${['success'].includes(n.type) ? 'text-emerald-500 bg-emerald-50' : 'text-blue-500 bg-blue-50'}`}>
            ...
            <div className="relative" ref={profileRef}>
              <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false); }}
                ...
                <div className="w-8 h-8 rounded-xl overflow-hidden shadow-sm ring-2 ring-emerald-500/30">
                ...
                <div className="hidden sm:block text-left">
                  ...
                  <div className="text-[10px] text-gray-400">{user?.franchiseCenter}</div>
                </div>
                ...
                {profileOpen && (
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    ...
                    <div className={`px-4 py-3 border-b ${dark ? 'border-gray-700' : 'border-gray-100'}`}>
                      <div className={`font-bold text-sm ${text}`}>{user?.name}</div>
                      <div className="text-xs text-gray-400">{user?.franchiseCenter}</div>
                    </div>
                    <div className="p-2">
                      <Link to="/franchise" className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-colors ${dark ? 'text-gray-300 hover:bg-gray-800' : 'text-gray-600 hover:bg-gray-50'}`}>
                        🏠 Dashboard
                      </Link>
